...
function reset_temporary($db_object){
    $sql1 = "TRUNCATE itemset1_apriori";
    $db_object->db_query($sql1);
    
    $sql2 = "TRUNCATE itemset2_apriori";
    $db_object->db_query($sql2);
    
    $sql3 = "TRUNCATE itemset3_apriori";
    $db_object->db_query($sql3);
    
    $sql4 = "TRUNCATE confidence_apriori";
    $db_object->db_query($sql4);
}

function reset_hitungan($db_object, $id_process){
    $condition = array("id_process"=>$id_process);
    $db_object->delete_record("itemset1_apriori", $condition);
    
    //$condition = array("id_process"=>$id_process);
    $db_object->delete_record("itemset2_apriori", $condition);
    
    //$condition = array("id_process"=>$id_process);
    $db_object->delete_record("itemset3_apriori", $condition);
    
    //$condition = array("id_process"=>$id_process);
    $db_object->delete_record("confidence_apriori", $condition);
}
...
function mining_process($db_object, $min_support, $min_confidence, $start_date, $end_date, $id_process){
    ...
    echo "</table>";
    //insert into itemset1_apriori one query with many value
    $value_insert = implode(",", $valueIn);
    $sql_insert_itemset1 = "INSERT INTO itemset1_apriori (atribut, jumlah, support, lolos, id_process) "
            . " VALUES ".$value_insert;
    ...
    $sql_insert_itemset2 = "INSERT INTO itemset2_apriori (atribut1, atribut2, jumlah, support, lolos, id_process) "
    ...
    echo "</table>";
    //insert into itemset3_apriori one query with many value
    $value_insert_itemset3 = implode(",", $valueIn_itemset3);
    $sql_insert_itemset3 = "INSERT INTO itemset3_apriori(atribut1, atribut2, atribut3, jumlah, support, lolos, id_process) "
    ...
    $sql_3 = "SELECT * FROM itemset3_apriori WHERE lolos = 1 AND id_process = ".$id_process;
    ...
    //dari itemset 2
    $sql_2 = "SELECT * FROM itemset2_apriori WHERE lolos = 1 AND id_process = ".$id_process;
...
//    while($row1_ = $db_object->db_fetch_array($res1_)){
    ...
    $nilai_support_x = ($jml_itemset2/$jumlah_transaksi) * 100;
        ...
        $db_object->insert_record("confidence_apriori", 
...
//        while($row4_ = $db_object->db_fetch_array($res4_)){
    ...
    $nilai_support_x = ($jml_itemset1/$jumlah_transaksi) * 100;
            ...
            //masukkan ke table confidence_apriori
            $db_object->insert_record("confidence_apriori", 
                    array("kombinasi1" => $kombinasi1,
                        "kombinasi2" => $kombinasi2,
                        "support_xUy" => $supp_xuy,
